...
function flyoutActive() {
    ...
    document.addEventListener('click', (e) => {
        if( !( e.target.closest('[data-flyout-button]') || e.target.closest('[data-flyout-target]')) ) {
    ...
    revealTargets.forEach( target => {
        ...
        if( target.closest('[data-reveal-container]') ) {
            ...
            subtargets.forEach( (subtarget, i) => {
                subtarget.style.transitionDelay = `${ (i+1) * 0.05}s`;
            })
        }
...
function numberItemCountUp(value, item) {
    ...
    let timeStep = animationDuration / value;
